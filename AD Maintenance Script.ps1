...
$InactivityDisable = "30.00:00:00"

#Inactivity threshhold for Delete - 60 days in my example# 
$InactivityDelete = "60.00:00:00"
...
$DeleteUsers = Search-ADAccount -searchbase $DisabledUsersOU -AccountInactive -TimeSpan $InactivityDelete -UsersOnly
foreach ($DeleteUser in $DeleteUsers){
    $sam=$DeleteUser.samaccountname
    $dn=$DeleteUser.distinguishedName
{
        $DeleteUser|Remove-ADAccount -confirm $false
        } }
...
$Csvfile1 = $Path + "\AllADComputers_$logDate1.csv"
$time = (Get-Date).Adddays(-($InactivityDelete))

Get-ADComputer -SearchBase $DisabledComputersOU -Filter {LastLogonTimeStamp -lt $time} -ResultPageSize 1000 -resultSetSize $null -Properties Name, SamAccountName|
Export-CSV -Encoding UTF8 –NoTypeInformation 

$DeleteComputers = Search-ADAccount -searchbase 'CN=Computers,DC=Adatum,DC=Com' -AccountInactive -TimeSpan $InactivityDelete -ComputersOnly
foreach ($DeleteComputer in $DeleteComputers){
    $sam=$DeleteComputer.samaccountname
    $dn=$DeleteComputer.distinguishedName

{   $DeleteComputer|Remove-ADAccount -confirm $false  
    }  }